        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card card-primary card-outline mb-4">
                    ...
                    <form action="{{ route('admin.expenseListUpdate', $expenseList->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            <div class="row">
                                        ...
                                        <select class="form-select @error('expense_head_id') is-invalid @enderror"
                                            ...
                                            @foreach($expenseHeads as $expenseHead)
                                                <option value="{{ $expenseHead->id }}"
                                                        ...
                                                        data-current-amount="{{ $expenseList->expense_head_id == $expenseHead->id ? $expenseList->amount : 0 }}"
                                                        {{ old('expense_head_id', $expenseList->expense_head_id) == $expenseHead->id ? 'selected' : '' }}>
                                                    {{ $expenseHead->title }}
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('title') is-invalid @enderror"
                                               ...
                                               value="{{ old('title', $expenseList->title) }}" required>
                                        @error('title')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('description') is-invalid @enderror"
                                                  ...
                                                  placeholder="Enter expense description">{{ old('description', $expenseList->description) }}</textarea>
                                        @error('description')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" step="0.01" min="0.01"
                                               ...
                                               value="{{ old('amount', $expenseList->amount) }}" required>
                                        @error('amount')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control @error('expense_date') is-invalid @enderror"
                                               ...
                                               value="{{ old('expense_date', $expenseList->expense_date->format('Y-m-d')) }}" required>
                                        @error('expense_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('reference_no') is-invalid @enderror"
                                               ...
                                               value="{{ old('reference_no', $expenseList->reference_no) }}">
                                        @error('reference_no')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                            ...
                                            <option value="1" {{ old('status', $expenseList->status) == '1' ? 'selected' : '' }}>Active</option>
                                            <option value="0" {{ old('status', $expenseList->status) == '0' ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                        @error('status')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                        ...
                        <div class="card-footer">
                            <button type="submit" class="btn btn-primary">
                                <span class="mdi mdi-content-save"></span> Update Expense
                            </button>
...
<script>
    $(document).ready(function() {
        ...
        $('#expense_head_id').on('change', function() {
            ...
            const currentAmount = parseFloat(selectedOption.data('current-amount')) || 0;

            // If editing and same expense head selected, add back current expense to remaining
            ...
            if (selectedOption.val()) {
                $('#budgetInfo').html(`
                    ...
                    Available (including current): ৳${adjustedRemaining.toFixed(2)}
                `);
            ...
            if (selectedOption.val() && amount > 0) {
                if (amount > adjustedRemaining) {
                    $('#amountWarning').html(`
                        ...
                        Warning: This amount exceeds the available budget by ৳${(amount - adjustedRemaining).toFixed(2)}
                    `);
                } else {
                    $('#amountWarning').html('');
                }
