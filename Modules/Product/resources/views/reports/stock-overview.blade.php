        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Stock Overview Report</h3>
                    <div class="card-tools">
                        <button type="button" class="btn btn-success btn-sm" onclick="exportTableToExcel('stockOverviewTable', 'Stock_Overview')">
                            ...
                            <tbody>
                                ...
                                @forelse($products as $key => $product)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $product['name'] }}</td>
                                        <td class="text-end">{{ number_format($product['total_purchase_qty'], 0) }}</td>
                                        <td class="text-end">৳{{ number_format($product['total_purchase_amount'], 2) }}</td>
                                        <td class="text-end">{{ number_format($product['total_sold_qty'], 0) }}</td>
                                        <td class="text-end">৳{{ number_format($product['total_sold_amount'], 2) }}</td>
                                        <td class="text-end">{{ number_format($product['available_qty'], 0) }}</td>
                                        <td class="text-end">৳{{ number_format($product['available_amount'], 2) }}</td>
                                    </tr>
                                @empty
                                    <tr>
